...
3. Access, command authorization and mention gating must precede media downloads, session writes and agent dispatch. Do not silently share DM session keys between unrelated users or accounts.
...
6. Persist a batch before acknowledging its cursor. Failed events must remain inspectable. Never reset a corrupt queue automatically, steal a lock, or claim exactly-once delivery. Mark turns started before possible side effects. Failed and interrupted turns are quarantined, never automatically replayed. Block newer messages only in the affected chat. Retried agent turns can repeat side effects.
